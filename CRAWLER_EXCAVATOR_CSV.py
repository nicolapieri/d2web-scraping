...
url = f"http://2fd6cemt4gmccflhm6imvdfvli3nf7zn6rfrwpsy7uhxrgbypvwf5fad.onion/search/{keywords}"
print(f"\nCurrently querying '{yourquery}' on EXCAVATOR search engine.")
...
filename = "excavator_crawling_results" + "_" + str(datetime.now().strftime("%Y-%m-%d") + "_" + datetime.now().strftime("%H.%M.%S")) + ".csv"
...
for link in onion_links:
    with open(savepath + filename,"a") as file:
        ...
        file.write(keywords + "," + "Excavator" + "," + str(datetime.now().strftime("%Y-%m-%d") + "_" + datetime.now().strftime("%H:%M:%S")) + "," + row + "\n")
